...
if(isset($_POST['update'])){
    ...
    $query = $con->prepare("
                             SELECT * FROM users WHERE usersUsername=:usersUsername;
        ");
...
if(isset($_POST['updateSchedule'])){
    ...
    $query = $con->prepare("
                             SELECT * FROM scheduled_appointment WHERE usersID=:usersID AND availableID=:availableID;
        ");
...
function checkLogin($con, $username, $password){
    $query = $con->prepare("
                            SELECT * FROM users WHERE usersUsername=:usersUsername AND usersPassword=:usersPassword;
            ");
    $query->bindParam(":usersUsername",$username);
    $query->bindParam(":usersPassword",$password);
    $query->execute();
...
function updateDetails($con, $usersID, $username, $password, $name, $email){
    $query = $con->prepare("
            UPDATE users SET usersUsername=:usersUsername, usersPassword=:usersPassword, usersName=:usersName, usersEmail=:usersEmail
...
function updateSchedule($con, $course, $cSection, $platform, $comment, $currentUserID, $currentAvailableID){
    $query = $con->prepare("
            UPDATE scheduled_appointment SET course=:course, cSection=:cSection, platform=:platform, comment=:comment
