...
function fetchRecords($con){
    $query = $con->prepare("
                        SELECT * FROM users WHERE usersUsername='{$_SESSION['usersUsername']}';
            ");
    $query->execute();
    return $query->fetch();
}
...
function fetchRecords2($con, $results){
    $query2 = $con->prepare("
                        SELECT * FROM scheduled_appointment WHERE usersID='{$results['usersID']}';
            ");
    $query2->execute();
    return $query2->fetch();
}
...
function fetchRecords3($con){
    $query3 = $con->prepare("
                        SELECT * FROM available_appointment WHERE available='0'
            ");
    $query3->execute();
...
function fetchRecords4($con, $results2){
    $query4 = $con->prepare("
                        SELECT * FROM available_appointment WHERE availableID='{$results2['availableID']}';
            ");
    $query4->execute();
    return $query4->fetch();
}
...
$_SESSION['usersID'] = $results['usersID'];
echo $_SESSION['usersID'];

$_SESSION['availableID'] = $results2['availableID'];
...
if ($results['accountID'] == 2){echo "<a href='deleteuser.php'>Delete User</a>";
echo "<br><br>";

if (isset($_SESSION['availableID'])){echo "<a href='updateappointment.php'>Update Appointment</a> &nbsp;";
...
echo "<br><br>"; } else {include("instructortable.html");}
?>
